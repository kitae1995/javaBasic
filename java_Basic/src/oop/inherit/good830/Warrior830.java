...
public class Warrior830 extends Player830 { // warrior830 클래스는 player830 클래스의 기능을 물려받음
	
	int rage;

	void characterInfo() { // 부모가 물려준 기능에 무언가를 추가하고 싶다면? 그냥 추가하는게 아니고 super(부모)를 붙여야함
		   // 이를 오버라이딩이라함.
		   // 그냥 부모 클래스에 있는 내용을 복붙해서 와도 되지만 지저분해짐
		   // 오버라이딩 단축키 Alt + shift + S로 불러올수있음 , 아니면 자동완성으로 바로 가능
		super.characterInfo();
