...
public class MethodExample2 {
	/*
	 ...
	 1. 매개변수는 메서드를 호출할 때, 메서드 실행에 필요한 값들을
	  메서드 내부로 전달하는 매개체 역할을 합니다. // kitae(int x) <-여기서 int x
	...
	static int calcRangeTotal(int start, int end){ // 매개변수 2개 주는 예제
		int total = 0;
	...
	//정수를 n개 전달하면 그 정수들의 총 합을 리턴하는 메서드
	static int calcNumberTotal(int[] nums) {
		int total = 0;
		for(int n:nums) { //nums에 있는 요소 하나하나씩 n에 들어감 for문처럼
			total+= n;
		}
		//for(int i=0; i<nums.length; i++){
		//	total += nums[i]; 와 같이 그냥 for문으로도 제작 가능
		//}
		return total;
	}
	...
	static int calcNumberTotal2(int ...nums) { // ... 을 붙이면 가변인수가 됨 가변인수가 되면 실행할때 매개변수를 줘도 됨
		int total = 0;
		for(int n : nums) {
			total += n;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("오늘의 점심은? " + selectRandomFood());
